...
class IrRollTemplate extends React.Component {
  render() {
        ...
        {posts &&
          posts.map(({ node: post }) => (
            <div className="is-parent column is-6" key={post.id}>
              <article className={`ir-list-item tile is-child box notification ${post.frontmatter.featuredpost ? "is-featured" : ""}`}>
                <header>
...
export default function IrRoll() {
  return (
    <StaticQuery
      query={graphql`
        query IrRollQuery {
      ...
      `}
      render={(data, count) => <IrRollTemplate data={data} count={count} />}
    />
  );
}
